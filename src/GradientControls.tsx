...
const StopPicker = () => {
  ...
  const handleMove = (newVal: string) => {
    handleGradient(currentColor, formatInputValues(Number(newVal), 0, 100));
  };
...
const DegreePicker = () => {
  ...
  const handleDegrees = (e: React.ChangeEvent<HTMLInputElement>) => {
    let newValue = formatInputValues(Number(e.target.value), 0, 360);
    const remaining = value.split(/,(.+)/)[1];
...
const DeleteBtn = () => {
  ...
  return (
    <div
      ...
      style={{ width: 30, ...controlBtnStyles(false), marginRight: 1 }}
    >
      <TrashIcon />
    </div>
  );
